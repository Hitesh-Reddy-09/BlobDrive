...
function FileGrid() {
  ...
  const getSortIcon = (sortBy) => {
    ...
    return state.sortOrder === 'asc' ? <ArrowUp size={16} /> : <ArrowDown size={16} />;
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${state.selectedFiles.length} selected item(s)?`)) return;
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.sortControls}>
          ...
          <button 
            className={`${styles.sortButton} ${state.sortBy === 'name' ? styles.active : ''}`}
            onClick={() => handleSort('name')}
          >
            Name {getSortIcon('name')}
          </button>
          <button 
            className={`${styles.sortButton} ${state.sortBy === 'modified' ? styles.active : ''}`}
            onClick={() => handleSort('modified')}
          >
            Modified {getSortIcon('modified')}
          </button>
          <button 
            className={`${styles.sortButton} ${state.sortBy === 'size' ? styles.active : ''}`}
      ...
      {state.viewMode === 'grid' ? (
        <div className={styles.grid}>
          {files.map((file) => (
            <div
              key={file.id}
              className={`${styles.fileCard} ${state.selectedFiles.includes(file.id) ? styles.selected : ''}`}
            ...
            >
              <div className={styles.fileCardHeader}>
                ...
                />
                <button className={styles.moreButton} onClick={e => { e.stopPropagation(); toggleDropdown(file._id || file.id); }}>
                  <MoreHorizontal size={16} />
                </button>
                {dropdownOpen[file._id || file.id] && (
                  <div className={styles.dropdownMenu} onClick={e => e.stopPropagation()}>
                    <button className={styles.dropdownItem} onClick={e => handleShare(file, e)}>
                      Share
                    </button>
                    {/* Add more options here if needed */}
                  </div>
                )}
                <button className={styles.moreButton} onClick={e => { e.stopPropagation(); handleDelete(file); }} title="Delete">
                  <Trash2 size={16} />
                </button>
              </div>
        ...
        <div className={styles.list}>
          ...
          {files.map((file) => (
            <div
              ...
              className={`${styles.listRow} ${state.selectedFiles.includes(file.id) ? styles.selected : ''}`}
              onClick={() => handleFileClick(file)}
            >
              <div className={styles.listColumn}>
                <div className={styles.listFileInfo}>
                  ...
                  <div>
                    ...
                    {file.azureBlobUrl && (
                      <a href={file.azureBlobUrl} target="_blank" rel="noopener noreferrer" className={styles.downloadLink}>
                        Download
                      </a>
                    )}
                    {file.tags.length > 0 && (
              ...
              <div className={styles.listColumn}>{formatDate(file.modifiedAt)}</div>
              <div className={styles.listColumn}>{formatFileSize(file.size)}</div>
              <div className={styles.listColumn}>{file.type === 'folder' ? 'Folder' : file.mimeType?.split('/')[1] || 'File'}</div>
              <div className={styles.listColumn}>
                <button className={styles.moreButton} onClick={e => { e.stopPropagation(); toggleDropdown(file._id || file.id); }}>
                  <MoreHorizontal size={16} />
                </button>
